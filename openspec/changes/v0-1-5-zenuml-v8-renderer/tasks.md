...
- [ ] 1.1 `@zenuml/core` の最新バージョンを確認し、`dist/zenuml.js`（CJS）を
  `crates/katana-canvas-forge/vendor/zenuml-core/<version>/zenuml.js` に配置する
- [ ] 1.2 `shasum -a 256 zenuml.js > zenuml.js.sha256` を実行して checksum を生成する
- [ ] 1.3 `Justfile` に `ZENUML_CORE_JS_VERSION` 変数と `verify-zenuml-core` recipe を追加する
  （`mermaid-zenuml` の `MERMAID_ZENUML_JS_VERSION` / `verify-zenuml` と同パターン）
- [ ] 1.4 `just verify-zenuml-core` が通ることを確認する
...
- [ ] 2.2 `Node.js` でブリッジ単体動作確認スクリプトを `tmp/` に作成して実行し、
...
- [ ] 3.1 `zenuml_v8_runtime.rs` を作成する
...
- [ ] 3.2 `ZenumlBrowserRenderOps` の呼び出し箇所を `ZenumlV8RenderOps` に置き換える
- [ ] 3.3 `cargo build` が通ることを確認する

---

## Phase 4: 旧実装の削除

- [ ] 4.1 `zenuml_browser_runtime.rs` を削除する
- [ ] 4.2 `zenuml_browser_runtime_tests.rs` を削除する
  （テストは Phase 5 で V8 版として再実装する）
- [ ] 4.3 `render_zenuml_browser.mjs` を削除する
- [ ] 4.4 `ProcessService` への `node` 依存が ZenUML 以外に残っていないか確認する
- [ ] 4.5 `cargo build` が通ることを確認する

---

## Phase 5: テストと品質ゲート

- [ ] 5.1 `zenuml_v8_runtime_tests.rs` を作成し、fixture の SVG が得られることを確認する
  - `<svg>` タグを含む
  - `<foreignObject>` を含まない
  - エラーなく完了する
...
- [ ] 5b.1 markdown パーサのコードブロック言語タグ処理を確認し、
  `zenuml` タグを `ZenumlV8RenderOps` に直接ルーティングする実装箇所を特定する
- [ ] 5b.2 `~~~zenuml` タグのルーティングを実装する
  （strip 不要のままそのまま `ZenumlV8RenderOps::render()` に渡す）
- [ ] 5b.3 `tests/fixtures/` に `~~~zenuml` 直接記法のサンプル fixture を追加する
- [ ] 5b.4 fixture を使った integration test を追加し、`cargo test` が通ることを確認する

---

## Phase 6: バージョン bump と仕上げ

- [ ] 6.1 workspace `Cargo.toml` の `version` を `0.1.4` → `0.1.5` に上げる
- [ ] 6.2 `CHANGELOG` または release note を更新する（該当する場合）
- [ ] 6.3 `just release-check` が通ることを確認する
- [ ] 6.4 `openspec/changes/v0-1-5-zenuml-v8-renderer/tasks.md` の全タスクが `[x]` であることを確認する
