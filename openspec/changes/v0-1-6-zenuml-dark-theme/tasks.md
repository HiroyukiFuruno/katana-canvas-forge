...
- [ ] 2.1 `build_preamble(source, is_dark: bool)` にシグネチャを変更する
  - `var __zenuml_dark__ = true/false;` を出力する
- [ ] 2.2 `ZenumlV8RenderOps::render()` 内の `_preset` → `preset` に変更し、
  `build_preamble(source, preset.dark_mode)` を呼ぶ
- [ ] 2.3 `cargo build` が通ることを確認する

---

## Phase 3: テストと品質ゲート

- [ ] 3.1 `zenuml_v8_runtime_tests.rs` にテストを追加する
  - ダークモードで `<svg>` が返ること
  - ライトモードで `<svg>` が返ること
  - `build_preamble` が `__zenuml_dark__` を正しく出力すること
- [ ] 3.2 `cargo test` が通ることを確認する
- [ ] 3.3 `just lint` が通ることを確認する

---

## Phase 4: バージョン bump と仕上げ

- [ ] 4.1 workspace `Cargo.toml` の `version` を `0.1.5` → `0.1.6` に上げる
- [ ] 4.2 `just VERSION=v0.1.6 release-check` が通ることを確認する
- [ ] 4.3 本 tasks.md の全タスクが `[x]` であることを確認する
